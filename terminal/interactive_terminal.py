...
from PySide6.QtCore import QProcess, Signal, Slot, Qt, QTimer
from PySide6.QtGui import QTextCursor, QTextCharFormat, QColor, QKeyEvent
import sys
import os

class TerminalInstance(QWidget):
    process_changed = Signal(str) # New signal for tab renaming

    def __init__(self, parent=None):
        super().__init__(parent)
        self.process = None
        self.current_process_command = None # To store the running command
        self.setup_ui()
        self.start_shell()
    ...
    def start_shell(self):
        ...
        try:
            ...
            if not self.process.waitForStarted(5000):
                ...
                self.process_changed.emit("Terminal") # Initial tab name
    ...
    def run_command(self, command):
        ...
        if self.process and self.process.state() == QProcess.Running:
            self.current_process_command = command
            self.process_changed.emit("Running: " + command)
